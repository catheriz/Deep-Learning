...
def visualize_training(inputs):
    with tf.name_scope('leaky-ReLu_Xavier_Adagrad'):
        tf.summary.scalar('Min', tf.math.reduce_min(inputs))
        tf.summary.scalar('Max',tf.math.reduce_max(inputs))
        tf.summary.scalar('Mean', tf.math.reduce_max(inputs))
        tf.summary.scalar('Std',tf.math.reduce_std(inputs))
        tf.summary.histogram('Histogram', inputs)
...
def main():
    ...
    visualize_training(W_conv1)
    visualize_training(b_conv1)
    visualize_training(h_conv1)
    visualize_training(h_pool1)
    ...
    visualize_training(W_conv2)
    visualize_training(b_conv2)
    visualize_training(h_conv2)
    visualize_training(h_pool2)
    # densely connected layer
    W_fc1 = weight_variable([7 * 7 * 64, 1024])
    b_fc1 = bias_variable([1024])
    h_pool2_flat = tf.reshape(h_pool2, [-1, 7 * 7 * 64])
    h_fc1 = tf.nn.leaky_relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1)
    visualize_training(W_fc1)
    visualize_training(b_fc1)
    visualize_training(h_pool2_flat)
    visualize_training(h_fc1)
    # dropout
    ...
    for i in range(max_step):
        ...
        if i % 1100 == 0 or i == max_step:
            ...
            print("validation error %g" % loss.eval(feed_dict={
    ...
    # print validation error and accuracy
    print("validation error %g" % loss.eval(feed_dict={
        x: mnist.validation.images, y_: mnist.validation.labels, keep_prob: 1.0}))
